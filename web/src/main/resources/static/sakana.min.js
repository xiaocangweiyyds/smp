(function (D, W) {
    typeof exports == "object" && typeof module == "object" ? module.exports = W() : typeof define == "function" && define.amd ? define("SakanaWidget", [], W) : typeof exports == "object" ? exports.SakanaWidget = W() : D.SakanaWidget = W()
})(self, () => (() => {
    "use strict";
    var M = {
        951: (l, n, c) => {
            c.d(n, {Z: () => w});
            var p = c(204), o = c.n(p), s = c(415), i = c.n(s), h = i()(o());
            h.push([l.id, ".sakana-widget-app{pointer-events:none;position:relative}.sakana-widget-canvas{z-index:10;pointer-events:none;position:absolute}.sakana-widget-main{z-index:20;pointer-events:none;position:absolute;display:flex;flex-direction:column;justify-content:space-between;align-items:center}.sakana-widget-img{z-index:40;cursor:move;pointer-events:auto;position:relative;background:no-repeat 50% 50%;background-size:cover}.sakana-widget-ctrl{z-index:30;cursor:pointer;pointer-events:auto;position:relative;height:24px;width:112px;display:flex;border-radius:4px;background-color:#ddd;box-shadow:0 8px 24px rgba(0,0,0,.1)}.sakana-widget-ctrl-item{height:24px;width:28px;display:flex;justify-content:center;align-items:center;color:#555;background-color:rgba(0,0,0,0)}.sakana-widget-ctrl-item:hover{color:#555;background-color:rgba(255,255,255,.25)}.sakana-widget-icon{height:18px;width:18px}.sakana-widget-icon--rotate{animation:sakana-widget-spin 2s linear infinite}@keyframes sakana-widget-spin{100%{transform:rotate(360deg)}}", ""]);
            const w = h
        }, 415: l => {
            l.exports = function (n) {
                var c = [];
                return c.toString = function () {
                    return this.map(function (o) {
                        var s = "", i = typeof o[5] != "undefined";
                        return o[4] && (s += "@supports (".concat(o[4], ") {")), o[2] && (s += "@media ".concat(o[2], " {")), i && (s += "@layer".concat(o[5].length > 0 ? " ".concat(o[5]) : "", " {")), s += n(o), i && (s += "}"), o[2] && (s += "}"), o[4] && (s += "}"), s
                    }).join("")
                }, c.i = function (o, s, i, h, w) {
                    typeof o == "string" && (o = [[null, o, void 0]]);
                    var Y = {};
                    if (i) for (var g = 0; g < this.length; g++) {
                        var K = this[g][0];
                        K != null && (Y[K] = !0)
                    }
                    for (var A = 0; A < o.length; A++) {
                        var d = [].concat(o[A]);
                        i && Y[d[0]] || (typeof w != "undefined" && (typeof d[5] == "undefined" || (d[1] = "@layer".concat(d[5].length > 0 ? " ".concat(d[5]) : "", " {").concat(d[1], "}")), d[5] = w), s && (d[2] && (d[1] = "@media ".concat(d[2], " {").concat(d[1], "}")), d[2] = s), h && (d[4] ? (d[1] = "@supports (".concat(d[4], ") {").concat(d[1], "}"), d[4] = h) : d[4] = "".concat(h)), c.push(d))
                    }
                }, c
            }
        }, 204: l => {
            l.exports = function (n) {
                return n[1]
            }
        }, 624: l => {
            var n = [];

            function c(s) {
                for (var i = -1, h = 0; h < n.length; h++) if (n[h].identifier === s) {
                    i = h;
                    break
                }
                return i
            }

            function p(s, i) {
                for (var h = {}, w = [], Y = 0; Y < s.length; Y++) {
                    var g = s[Y], K = i.base ? g[0] + i.base : g[0], A = h[K] || 0, d = "".concat(K, " ").concat(A);
                    h[K] = A + 1;
                    var x = c(d), B = {css: g[1], media: g[2], sourceMap: g[3], supports: g[4], layer: g[5]};
                    if (x !== -1) n[x].references++, n[x].updater(B); else {
                        var E = o(B, i);
                        i.byIndex = Y, n.splice(Y, 0, {identifier: d, updater: E, references: 1})
                    }
                    w.push(d)
                }
                return w
            }

            function o(s, i) {
                var h = i.domAPI(i);
                h.update(s);
                var w = function (g) {
                    if (g) {
                        if (g.css === s.css && g.media === s.media && g.sourceMap === s.sourceMap && g.supports === s.supports && g.layer === s.layer) return;
                        h.update(s = g)
                    } else h.remove()
                };
                return w
            }

            l.exports = function (s, i) {
                i = i || {}, s = s || [];
                var h = p(s, i);
                return function (Y) {
                    Y = Y || [];
                    for (var g = 0; g < h.length; g++) {
                        var K = h[g], A = c(K);
                        n[A].references--
                    }
                    for (var d = p(Y, i), x = 0; x < h.length; x++) {
                        var B = h[x], E = c(B);
                        n[E].references === 0 && (n[E].updater(), n.splice(E, 1))
                    }
                    h = d
                }
            }
        }, 892: l => {
            var n = {};

            function c(o) {
                if (typeof n[o] == "undefined") {
                    var s = document.querySelector(o);
                    if (window.HTMLIFrameElement && s instanceof window.HTMLIFrameElement) try {
                        s = s.contentDocument.head
                    } catch (i) {
                        s = null
                    }
                    n[o] = s
                }
                return n[o]
            }

            function p(o, s) {
                var i = c(o);
                if (!i) throw new Error("Couldn't find a style target. This probably means that the value for the 'insert' parameter is invalid.");
                i.appendChild(s)
            }

            l.exports = p
        }, 716: l => {
            function n(c) {
                var p = document.createElement("style");
                return c.setAttributes(p, c.attributes), c.insert(p, c.options), p
            }

            l.exports = n
        }, 359: (l, n, c) => {
            function p(o) {
                var s = c.nc;
                s && o.setAttribute("nonce", s)
            }

            l.exports = p
        }, 990: l => {
            function n(o, s, i) {
                var h = "";
                i.supports && (h += "@supports (".concat(i.supports, ") {")), i.media && (h += "@media ".concat(i.media, " {"));
                var w = typeof i.layer != "undefined";
                w && (h += "@layer".concat(i.layer.length > 0 ? " ".concat(i.layer) : "", " {")), h += i.css, w && (h += "}"), i.media && (h += "}"), i.supports && (h += "}");
                var Y = i.sourceMap;
                Y && typeof btoa != "undefined" && (h += `
/*# sourceMappingURL=data:application/json;base64,`.concat(btoa(unescape(encodeURIComponent(JSON.stringify(Y)))), " */")), s.styleTagTransform(h, o, s.options)
            }

            function c(o) {
                if (o.parentNode === null) return !1;
                o.parentNode.removeChild(o)
            }

            function p(o) {
                var s = o.insertStyleElement(o);
                return {
                    update: function (h) {
                        n(s, o, h)
                    }, remove: function () {
                        c(s)
                    }
                }
            }

            l.exports = p
        }, 230: l => {
            function n(c, p) {
                if (p.styleSheet) p.styleSheet.cssText = c; else {
                    for (; p.firstChild;) p.removeChild(p.firstChild);
                    p.appendChild(document.createTextNode(c))
                }
            }

            l.exports = n
        }
    }, D = {};

    function W(l) {
        var n = D[l];
        if (n !== void 0) return n.exports;
        var c = D[l] = {id: l, exports: {}};
        return M[l](c, c.exports, W), c.exports
    }

    W.n = l => {
        var n = l && l.__esModule ? () => l.default : () => l;
        return W.d(n, {a: n}), n
    }, W.d = (l, n) => {
        for (var c in n) W.o(n, c) && !W.o(l, c) && Object.defineProperty(l, c, {enumerable: !0, get: n[c]})
    }, W.o = (l, n) => Object.prototype.hasOwnProperty.call(l, n), W.nc = void 0;
    var J = {};
    return (() => {
        W.d(J, {default: () => X});
        var l = W(624), n = W.n(l), c = W(990), p = W.n(c), o = W(892), s = W.n(o), i = W(359), h = W.n(i), w = W(716),
            Y = W.n(w), g = W(230), K = W.n(g), A = W(951), d = {};
        d.styleTagTransform = K(), d.setAttributes = h(), d.insert = s().bind(null, "head"), d.domAPI = p(), d.insertStyleElement = Y();
        var x = n()(A.Z, d);
        const B = A.Z && A.Z.locals ? A.Z.locals : void 0, O = {
                chisato: {
                    image: "data:image/png;base64,",
                    initialState: {i: .08, s: .1, d: .99, r: 1, y: 40, t: 0, w: 0}
                }, takina: {
                    image: "data:image/png;base64,",
                    initialState: {i: .08, s: .1, d: .988, r: 12, y: 2, t: 0, w: 0}
                }
            },
            N = '<svg xmlns="http://www.w3.org/2000/svg" class="sakana-widget-icon" viewBox="0 0 512 512"><path d="M448 256c0-106-86-192-192-192S64 150 64 256s86 192 192 192 192-86 192-192z" fill="none" stroke="currentColor" stroke-miterlimit="10" stroke-width="32"/><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="32" d="M320 320 192 192m0 128 128-128"/></svg>',
            R = '<svg xmlns="http://www.w3.org/2000/svg" class="sakana-widget-icon" viewBox="0 0 512 512"><path fill="currentColor" d="M256 32C132.3 32 32 134.9 32 261.7c0 101.5 64.2 187.5 153.2 217.9a17.56 17.56 0 0 0 3.8.4c8.3 0 11.5-6.1 11.5-11.4 0-5.5-.2-19.9-.3-39.1a102.4 102.4 0 0 1-22.6 2.7c-43.1 0-52.9-33.5-52.9-33.5-10.2-26.5-24.9-33.6-24.9-33.6-19.5-13.7-.1-14.1 1.4-14.1h.1c22.5 2 34.3 23.8 34.3 23.8 11.2 19.6 26.2 25.1 39.6 25.1a63 63 0 0 0 25.6-6c2-14.8 7.8-24.9 14.2-30.7-49.7-5.8-102-25.5-102-113.5 0-25.1 8.7-45.6 23-61.6-2.3-5.8-10-29.2 2.2-60.8a18.64 18.64 0 0 1 5-.5c8.1 0 26.4 3.1 56.6 24.1a208.21 208.21 0 0 1 112.2 0c30.2-21 48.5-24.1 56.6-24.1a18.64 18.64 0 0 1 5 .5c12.2 31.6 4.5 55 2.2 60.8 14.3 16.1 23 36.6 23 61.6 0 88.2-52.4 107.6-102.3 113.3 8 7.1 15.2 21.1 15.2 42.5 0 30.7-.3 55.5-.3 63 0 5.4 3.1 11.5 11.4 11.5a19.35 19.35 0 0 0 4-.4C415.9 449.2 480 363.1 480 261.7 480 134.9 379.7 32 256 32z"/></svg>',
            G = '<svg xmlns="http://www.w3.org/2000/svg" class="sakana-widget-icon" viewBox="0 0 512 512"><path fill="currentColor" d="M258.9 48C141.92 46.42 46.42 141.92 48 258.9c1.56 112.19 92.91 203.54 205.1 205.1 117 1.6 212.48-93.9 210.88-210.88C462.44 140.91 371.09 49.56 258.9 48zm126.42 327.25a4 4 0 0 1-6.14-.32 124.27 124.27 0 0 0-32.35-29.59C321.37 329 289.11 320 256 320s-65.37 9-90.83 25.34a124.24 124.24 0 0 0-32.35 29.58 4 4 0 0 1-6.14.32A175.32 175.32 0 0 1 80 259c-1.63-97.31 78.22-178.76 175.57-179S432 158.81 432 256a175.32 175.32 0 0 1-46.68 119.25z"/><path fill="currentColor" d="M256 144c-19.72 0-37.55 7.39-50.22 20.82s-19 32-17.57 51.93C191.11 256 221.52 288 256 288s64.83-32 67.79-71.24c1.48-19.74-4.8-38.14-17.68-51.82C293.39 151.44 275.59 144 256 144z"/></svg>',
            P = '<svg xmlns="http://www.w3.org/2000/svg" class="sakana-widget-icon" viewBox="0 0 512 512"><path d="M448 256c0-106-86-192-192-192S64 150 64 256s86 192 192 192 192-86 192-192z" fill="none" stroke="currentColor" stroke-miterlimit="10" stroke-width="32"/><path d="M351.82 271.87v-16A96.15 96.15 0 0 0 184.09 192m-24.2 48.17v16A96.22 96.22 0 0 0 327.81 320" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="32"/><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="32" d="m135.87 256 23.59-23.6 24.67 23.6m192 0-23.59 23.6-24.67-23.6"/></svg>';

        function k(v) {
            const t = typeof v;
            return v != null && (t === "object" || t === "function")
        }

        function I(v) {
            return typeof window.structuredClone == "function" ? window.structuredClone(v) : JSON.parse(JSON.stringify(v))
        }

        function q(v, t) {
            const e = I(v), a = I(t);
            return !k(e) || !k(a) || Object.keys(a).forEach(r => {
                k(a[r]) ? (k(e[r]) || (e[r] = {}), e[r] = q(e[r], a[r])) : e[r] = a[r]
            }), e
        }

        function Q(v, t, e = (window.devicePixelRatio || 1) * 2) {
            const a = t * e;
            v.width = a, v.height = a;
            const r = v.getContext("2d");
            return r ? (r.scale(e, e), r) : null
        }

        const T = {size: 200, character: "chisato", controls: !0, stroke: {color: "#b4b4b4", width: 10}}, f = {};
        Object.keys(O).forEach(v => {
            const t = O[v];
            f[v] = I(t)
        });

        class U {
            constructor(t = {}) {
                this._lastRunUnix = Date.now(), this._frameUnix = 1e3 / 60, this._running = !0, this._magicForceTimeout = 0, this._magicForceEnabled = !1, this._options = I(T), this._options = q(this._options, t), this._imageSize = this._options.size / 1.25, this._updateLimit(this._options.size), this.setCharacter(this._options.character), this._updateDom(), this._updateLimit = this._updateLimit.bind(this), this._updateDom = this._updateDom.bind(this), this._calcCenterPoint = this._calcCenterPoint.bind(this), this._draw = this._draw.bind(this), this._run = this._run.bind(this), this._move = this._move.bind(this), this._onMouseDown = this._onMouseDown.bind(this), this._onTouchStart = this._onTouchStart.bind(this), this._magicForce = this._magicForce.bind(this), this.setState = this.setState.bind(this), this.setCharacter = this.setCharacter.bind(this), this.nextCharacter = this.nextCharacter.bind(this), this.triggetAutoMode = this.triggetAutoMode.bind(this), this.mount = this.mount.bind(this), this.unmount = this.unmount.bind(this)
            }

            static getCharacter(t) {
                return f[t] || null
            }

            static getCharacters() {
                return f
            }

            static registerCharacter(t, e) {
                f[t] = I(e)
            }

            _updateLimit(t) {
                let e = t / 5;
                e < 30 ? e = 30 : e > 60 && (e = 60);
                const a = t / 4, r = -a;
                this._limit = {maxR: e, maxY: a, minY: r}
            }

            _updateDom() {
                const {size: t, controls: e} = this._options, a = this._imageSize, r = document.createElement("div");
                r.className = "sakana-widget-app", r.style.width = `${t}px`, r.style.height = `${t}px`, this._domApp = r;
                const C = document.createElement("canvas");
                C.className = "sakana-widget-canvas", C.style.width = `${t}px`, C.style.height = `${t}px`;
                const m = Q(C, t);
                if (!m) throw new Error("Invalid canvas context");
                r.appendChild(C), this._domCanvas = C, this._domCanvasCtx = m;
                const F = document.createElement("div");
                F.className = "sakana-widget-main", F.style.width = `${t}px`, F.style.height = `${t}px`, r.appendChild(F), this._domMain = F;
                const u = document.createElement("div");
                u.className = "sakana-widget-img", u.style.width = `${a}px`, u.style.height = `${a}px`, u.style.transformOrigin = `50% ${t}px`, u.style.backgroundImage = `url('${this._image}')`, F.appendChild(u), this._domImage = u;
                const V = document.createElement("div");
                V.className = "sakana-widget-ctrl", e && F.appendChild(V), this._domCtrl = V;
                const L = "sakana-widget-ctrl-item", y = document.createElement("div");
                y.className = L, y.innerHTML = G, V.appendChild(y), this._domCtrlPerson = y;
                const S = document.createElement("div");
                S.className = L, S.innerHTML = P, V.appendChild(S), this._domCtrlMagic = S;
                const b = document.createElement("a");
                b.className = L, b.href = "", b.target = "_blank", b.innerHTML = R, V.appendChild(b), this._domCtrlGitHub = b;
                const Z = document.createElement("div");
                Z.className = L, Z.innerHTML = N, V.appendChild(Z), this._domCtrlClose = Z
            }

            _calcCenterPoint(t, e, a, r) {
                const C = Math.PI / 180 * t, m = Math.cos(C), F = Math.sin(C), u = F * e + m * a - F * r,
                    V = m * e - m * r - F * a;
                return {nx: u, ny: V}
            }

            _draw() {
                const {r: t, y: e} = this._state, {size: a, controls: r, stroke: C} = this._options, m = this._domImage,
                    F = this._imageSize, u = this._domCanvasCtx, V = t * 1;
                m.style.transform = `rotate(${t}deg) translateX(${V}px) translateY(${e}px)`, u.clearRect(0, 0, a, a), u.save(), u.translate(a / 2, a), u.strokeStyle = C.color, u.lineWidth = C.width, u.lineCap = "round", u.beginPath(), r ? u.moveTo(0, -10) : u.moveTo(0, 10);
                const L = a - F / 2, {nx: y, ny: S} = this._calcCenterPoint(t, L, V, e);
                u.lineTo(y, -S), u.stroke(), u.restore()
            }

            _run() {
                if (!this._running) return;
                let {r: a, y: r, t: C, w: m} = this._state;
                const {d: F, i: u} = this._state, V = Date.now(), L = V - this._lastRunUnix;
                let y = u;
                if (L < 40 && (y = u / this._frameUnix * L), this._lastRunUnix = V, m = m - a * 2 - 0, a = a + m * y * 1.2, this._state.w = m * F, this._state.r = a, C = C - r * 2, r = r + C * y * 2, this._state.t = C * F, this._state.y = r, Math.max(Math.abs(this._state.w), Math.abs(this._state.r), Math.abs(this._state.t), Math.abs(this._state.y)) < .1) {
                    this._running = !1;
                    return
                }
                this._draw(), requestAnimationFrame(this._run)
            }

            _move(t, e) {
                const {maxR: a, maxY: r, minY: C} = this._limit;
                let m = t * this._state.s;
                m = Math.max(-a, m), m = Math.min(a, m), e = e * this._state.s * 2, e = Math.max(C, e), e = Math.min(r, e), this._state.r = m, this._state.y = e, this._state.w = 0, this._state.t = 0, this._draw()
            }

            _onMouseDown(t) {
                t.preventDefault(), this._running = !1;
                const {pageY: e} = t, a = e;
                this._state.w = 0, this._state.t = 0;
                const r = m => {
                    const F = this._domMain.getBoundingClientRect(), u = F.left + F.width / 2, {pageX: V, pageY: L} = m,
                        y = V - u, S = L - a;
                    this._move(y, S)
                }, C = () => {
                    document.removeEventListener("mousemove", r), document.removeEventListener("mouseup", C), this._running = !0, requestAnimationFrame(this._run)
                };
                document.addEventListener("mousemove", r), document.addEventListener("mouseup", C)
            }

            _onTouchStart(t) {
                if (t.preventDefault(), this._running = !1, !t.touches[0]) return;
                const {pageY: e} = t.touches[0], a = e;
                this._state.w = 0, this._state.t = 0;
                const r = m => {
                    if (!m.touches[0]) return;
                    const F = this._domMain.getBoundingClientRect(), u = F.left + F.width / 2, {
                        pageX: V,
                        pageY: L
                    } = m.touches[0], y = V - u, S = L - a;
                    this._move(y, S)
                }, C = () => {
                    document.removeEventListener("touchmove", r), document.removeEventListener("touchend", C), this._running = !0, requestAnimationFrame(this._run)
                };
                document.addEventListener("touchmove", r), document.addEventListener("touchend", C)
            }

            _magicForce() {
                if (Math.random() < .1) {
                    const t = Object.keys(f), e = Math.floor(Math.random() * t.length), a = t[e];
                    this.setCharacter(a)
                } else this._state.t = this._state.t + (Math.random() - .5) * 150, this._state.w = this._state.w + (Math.random() - .5) * 200;
                this._running || (this._running = !0, requestAnimationFrame(this._run)), this._magicForceTimeout = window.setTimeout(this._magicForce, Math.random() * 3e3 + 2e3)
            }

            setState(t) {
                return this._state || (this._state = {}), this._state = q(this._state, I(t)), this
            }

            setCharacter(t) {
                const e = f[t];
                if (!e) throw new Error(`invalid character ${t}`);
                return this._char = t, this._image = e.image, this.setState(e.initialState), this._domImage && (this._domImage.style.backgroundImage = `url('${this._image}')`), this
            }

            nextCharacter() {
                const t = Object.keys(U.getCharacters()).sort(), a = (t.indexOf(this._char) + 1) % t.length, r = t[a];
                return this.setCharacter(r), this
            }

            triggetAutoMode() {
                this._magicForceEnabled = !this._magicForceEnabled;
                const t = this._domCtrlMagic.querySelector("svg");
                this._magicForceEnabled ? t.classList.add("sakana-widget-icon--rotate") : t.classList.remove("sakana-widget-icon--rotate"), clearTimeout(this._magicForceTimeout), this._magicForceEnabled && (this._magicForceTimeout = window.setTimeout(this._magicForce, Math.random() * 1e3 + 500))
            }

            mount(t) {
                let e = null;
                if (typeof t == "string" && (e = document.querySelector(t)), !e) throw new Error("Invalid mounting element");
                const a = e.parentNode;
                if (!a) throw new Error("Invalid mounting element parent");
                this._domImage.addEventListener("mousedown", this._onMouseDown), this._domImage.addEventListener("touchstart", this._onTouchStart), this._domCtrlPerson.addEventListener("click", this.nextCharacter), this._domCtrlMagic.addEventListener("click", this.triggetAutoMode), this._domCtrlClose.addEventListener("click", this.unmount);
                const r = e.cloneNode(!1);
                return r.appendChild(this._domApp), a.replaceChild(r, e), requestAnimationFrame(this._run), this
            }

            unmount() {
                this._domImage.removeEventListener("mousedown", this._onMouseDown), this._domImage.removeEventListener("touchstart", this._onTouchStart), this._domCtrlPerson.removeEventListener("click", this.nextCharacter), this._domCtrlMagic.removeEventListener("click", this.triggetAutoMode), this._domCtrlClose.removeEventListener("click", this.unmount);
                const t = this._domApp.parentNode;
                if (!t) throw new Error("Invalid mounting element");
                return t.removeChild(this._domApp), this
            }
        }

        const X = U
    })(), J = J.default, J
})());
